...
class Main extends CI_Controller {
    ...
    public function course($id) {
        ...
        for ($i=0; $i<$numberOfSessions; $i++)
        {
            ...
            $sessions[$i] = $startTimes[$i] . ' to ' . $endTimes[$i] . ' - ' . $termName . ' - Day ' . $days[$i] . ' - ' . $buildings[$i] . ' (room ' . $rooms[$i] . ')';
        }
    ...
    public function browse() {
		...
		$this->load->view('templates/header', array('output' => 'Browse By'));
...
//		$this->load->view('browse', array('output' => $output));
    ...
    public function search() {
		...
		$this->load->view('templates/header', array('output' => 'Search'));
	...
	public function resultslist() {
		/** 
		 * Takes in search parameters from POST array, reassigns them into a new associative array, then passes this array into
		 * search method to return a list of courses that match search criteria.
		 */
		...
		if ($_POST['begin'] !== "standard") {
			$begin = $this->input->post('begin');
		}
		if ($_POST['day'] !== "standard") {
    ...
    public function schedule() {
	 ...
	 public function browseby($parameter) {
        /**
         * Gets list of Departments or Faculty or Gen Ed Departments based on input, then passes on
         * list to Categorylist View
         */
        
        $categoryList = $this->Courses->FetchByCategory($parameter);
